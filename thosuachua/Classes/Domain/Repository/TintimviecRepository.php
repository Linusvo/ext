...
/**
 * The repository for Tintimviecs
 */
class TintimviecRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
    ...
    function findByAllParam($param) {
        ...
        if($param['thanhpho'] < 999){
            $constraints[] =  $query->equals('thanhpho', $param['thanhpho']);
        }
    ...
    function findBySearchQuanhuyen($param) {
        $query = $this->createQuery();
        $query->matching(
            $query->logicalAnd(
                $query->equals('unterkategorie', $param['unterkategorienghanh']),
                $query->equals('quanhuyen1', $param['quanhuyen']),
                $query->equals('hidden1', 0)
            )
        );
        $query->setOrderings(array(
    ...
    function findBySearchQuanhuyen1($param) {
        $query = $this->createQuery();
        $query->matching(
            $query->logicalAnd(
                $query->equals('quanhuyen1', $param['quanhuyen']),
    ...
    function findBySearchThanhpho($param) {
        $query = $this->createQuery();
        $query->matching(
            $query->logicalAnd(
                $query->equals('unterkategorie', $param['unterkategorienghanh']),
                $query->equals('thanhpho', $param['thanhpho']),
                $query->equals('hidden1', 0)
            )
        );
        $query->setOrderings(array(
    ...
    function findBySearchThanhpho2($param) {
        $query = $this->createQuery();
        $query->matching(
            $query->logicalAnd(
                $query->equals('thanhpho', $param['thanhpho']),
    ...
    function findBySearchThanhpho1($param) {
        $query = $this->createQuery();
        $query->matching(
            $query->logicalAnd(
                $query->equals('unterkategorie', $param['unterkategorienghanh']),
