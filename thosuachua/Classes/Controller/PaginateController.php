...
class PaginateController extends \TYPO3\CMS\Fluid\Core\Widget\AbstractWidgetController  {
    ...
    public function indexAction($currentPage = 1, $itemsPerPage = 0) {
        ...
        if(isset($_POST[tx_thosuachua_thotimviecsearch][searchparam][unterkategorienghanh])){
            $unterkategorienghanh = $_POST[tx_thosuachua_thotimviecsearch][searchparam][unterkategorienghanh];
        }
        if(isset($_POST[tx_thosuachua_thotimviecsearch][searchparam][quanhuyen])){
            $quanhuyen = $_POST[tx_thosuachua_thotimviecsearch][searchparam][quanhuyen];
        }
        if(isset($_POST[tx_thosuachua_thotimviecsearch][searchparam][tinhthanhpho])){
            $tinhthanhpho = $_POST[tx_thosuachua_thotimviecsearch][searchparam][tinhthanhpho];
        }
        if(isset($_POST[tx_thosuachua_viectimthosearch][searchparam][unterkategorienghanh])){
            $unterkategorienghanh = $_POST[tx_thosuachua_viectimthosearch][searchparam][unterkategorienghanh];
        }
        if(isset($_POST[tx_thosuachua_viectimthosearch][searchparam][quanhuyen])){
            $quanhuyen = $_POST[tx_thosuachua_viectimthosearch][searchparam][quanhuyen];
        }
        if(isset($_POST[tx_thosuachua_viectimthosearch][searchparam][tinhthanhpho])){
            $tinhthanhpho = $_POST[tx_thosuachua_viectimthosearch][searchparam][tinhthanhpho];
        }
        $this->view->assign('unterkategorienghanh',$unterkategorienghanh );
        $this->view->assign('tinhthanhpho', $tinhthanhpho);
        $this->view->assign('quanhuyen', $quanhuyen);


        $this->view->assign('itemsPerPage', $this->itemsPerPage);

        $this->view->assign('summeelement', count($this->objects) +2000);

        if($currentPage == 1){
                if(count($this->objects) > 10){
                    $this->view->assign('anfangpagi',  1);
                    $this->view->assign('summeelementak',  10);
                }else{
                    $this->view->assign('anfangpagi',  1);
                    $this->view->assign('summeelementak', count($this->objects));
                }
        }else{
            $anfangpagi = 10*((integer)$currentPage-1)+1;
            $anfangpagis = 10*((integer)$currentPage);
            $this->view->assign('anfangpagi',  $anfangpagi);

            if(count($this->objects) > $anfangpagis){
                $this->view->assign('summeelementak',  $anfangpagis);
            }else{
                $this->view->assign('summeelementak', count($this->objects));
            }
        }
